...
- Demo star shape: the browser demo now includes a 5-pointed star (concave polygon, 10 vertices,
  outer radius 190 / inner radius 80) as a third shape option alongside circle and donut. A
  `<select id="shape">` control switches between all three; `polygon()` is used for the star region
  and its outline is drawn via `ctx.moveTo`/`lineTo` over the same vertex list.
